...
async function name(): Promise<number> {
  const { age, whight, hight, gender } = await get([
    "age",
    "whight",
    "hight",
    "gender",
  ]);
  let equation: number = +whight / +hight;
  if(age>=20){
...
function bmicalcabove20(x: number) {
	if(x<16){
